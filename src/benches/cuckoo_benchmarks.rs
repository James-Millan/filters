...
fn bench_cuckoo_filter_uniform_member(c: &mut Criterion) {
    // setup
    let cuckoo_filter = RefCell::new(cuckoofilter::CuckooFilter::new(SAMPLE_SIZE as usize, SAMPLE_SIZE as usize, 2));
...
criterion_group!(benches, bench_cuckoo_filter_uniform_member, bench_cuckoo_filter_disjoint_member,
    bench_cuckoo_filter_mixed_member, bench_cuckoo_filter_random_member, bench_cuckoo_filter_create);
