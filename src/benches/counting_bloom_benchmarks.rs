...
fn bench_counting_bloom_filter_uniform_member(c: &mut Criterion) {
    // setup
    ...
    c.bench_function("bench_counting_bloom_filter_uniform_member", |b| {
        b.iter_custom(|iters| {
...
fn bench_counting_bloom_filter_mixed_member(c: &mut Criterion) {
    ...
    c.bench_function("bench_counting_bloom_filter_mixed_member", |b| {
        b.iter_custom(|iters| {
            ...
            for _ in 0..num_runs {
                for i in 0..(SAMPLE_SIZE as usize) {
                    black_box(counting_bloom_filter.borrow().member(mixed_keys.1[i]));
                }
            }
            return start.elapsed();
        });
...
fn bench_counting_bloom_filter_disjoint_member(c: &mut Criterion) {
    ...
    c.bench_function("bench_counting_bloom_filter_disjoint_member", |b| {
        b.iter_custom(|iters| {
            ...
            for _ in 0..num_runs {
                for i in 0..(SAMPLE_SIZE as usize) {
                    black_box(counting_bloom_filter.borrow().member(disjoint_keys.1[i]));
                }
            }
            return start.elapsed();
        });
...
fn bench_counting_bloom_filter_random_member(c: &mut Criterion) {
    ...
    c.bench_function("bench_counting_bloom_filter_random_member", |b| {
        b.iter_custom(|iters| {
            ...
            for _ in 0..num_runs {
                for i in 0..(SAMPLE_SIZE as usize) {
                    black_box(counting_bloom_filter.borrow().member(random_keys.1[i]));
                    // counting_bloom_filter.borrow().member(random_keys.1[i]);
                }
            }
            return start.elapsed();
        });
...
fn bench_counting_bloom_filter_create(c: &mut Criterion) {
    ...
    c.bench_function("bench_counting_bloom_filter_create", |b| {
        b.iter_custom(|iters| {
            let start = Instant::now();
            for _ in 0..iters {
                let mut counting_bloom_filter = countingbloomfilter::CountingBloomFilter::new(SAMPLE_SIZE, 0.01);
                for i in 0..(SAMPLE_SIZE as usize) {
                    black_box(counting_bloom_filter.insert(random_keys.1[i]));
                }
            }
            return start.elapsed();
        });
...
criterion_group!(benches, bench_counting_bloom_filter_uniform_member, bench_counting_bloom_filter_disjoint_member,
    bench_counting_bloom_filter_mixed_member, bench_counting_bloom_filter_random_member,bench_counting_bloom_filter_create);
